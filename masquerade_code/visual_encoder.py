...
class VisualEncoder(nn.Module):
    ...
    def __init__(self, device, lr, model_name="resnet50", l2dist=True,
                 bc_weight=1.0, l2weight=0.0, l1weight=0.0, pretrained=False, use_film_cond=False,
        ...
        self.l2dist = l2dist  # Use L2 distance (True) or cosine similarity (False)
        self.pretrained = pretrained  # Whether to use pretrained visual encoder
        self.num_negatives = 3  # Number of negative samples for contrastive learning
        self.use_film_cond = use_film_cond  # Enable FiLM conditioning
        self.layernorm_finetune = layernorm_finetune  # Enable LayerNorm fine-tuning
        ...
        self.cs = torch.nn.CosineSimilarity(1)  # Cosine similarity for contrastive learning
        self.bce = nn.BCELoss(reduce=False)  # Binary cross-entropy loss
        self.sigm = Sigmoid()  # Sigmoid activation
        self.bc_loss = nn.MSELoss(reduction='none')  # MSE loss for behavioral cloning

        print('pretrained', self.pretrained)
        if self.layernorm_finetune:
            print('LayerNorm fine-tuning enabled - freezing all parameters except LayerNorm')
        ...
        if model_name == "resnet18":
            ...
            self.encoder = torchvision.models.resnet18(pretrained=self.pretrained)
            self.normlayer = transforms.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD)
            ...
            self.encoder = torchvision.models.resnet50(pretrained=self.pretrained)
            self.normlayer = transforms.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD)
        elif model_name == "vit":
            ...
            import timm
            self.outdim = 768
            self.encoder = timm.create_model('vit_base_patch16_224.mae', pretrained=True)
            self.normlayer = transforms.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD)
        ...
        if self.layernorm_finetune and "vit" in model_name:
            ...
            params += list(self.encoder.parameters())  # Add encoder parameters to optimizer
        ...
        if self.use_film_cond:
            ...
            self.film_network = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(film_hidden_dim, 2 * self.outdim)  # 2 * outdim for scale and bias
            ).to(self.device)
    ...
    def print_trainable_params_info(self):
        ...
        if self.layernorm_finetune:
            print("LayerNorm fine-tuning mode - only LayerNorm parameters are trainable")
    ...
    def generate_film_params(self, lang_embedding):
        ...
        if not self.use_film_cond:
            raise ValueError("FiLM conditioning is not enabled. Set use_film_cond=True in __init__")

        return self.film_network(lang_embedding)

    def forward(self, obs, num_ims=1, obs_shape=[3, 224, 224], lang_embedding=None):
        """
        ...
        if self.use_film_cond and lang_embedding is not None:
            ...
            if cond_embed.shape[-1] != 2 * self.outdim:
                raise ValueError(f"cond_embed should have shape [..., {2 * self.outdim}], got {cond_embed.shape}")
